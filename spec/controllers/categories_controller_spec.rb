...
describe CategoriesController do 
  ...
  describe 'new category action' do
    context 'logged in' do
      ...
      it 'lets user create a category that is unique to them' do
        ...
        retrievedUser = User.find_by(:username => "user1")
        retrievedUser2 = User.find_by(:username => "user2")
        retrievedCategory = Category.find_by(:name => "category1")
        expect(retrievedCategory).to be_instance_of(Category)
        expect(retrievedCategory.user_id).to eq(retrievedUser.id)
        expect(retrievedCategory.user_id).not_to eq(retrievedUser2.id)
        expect(page.status_code).to eq(200)
      end
    ...
    context 'logged out' do
      it 'does not let a user view new category form' do 
        get '/categories/new'
  ...
  describe 'categories index action' do
    context 'logged in' do 
      ...
      it 'lets a user view their categories index' do 
        visit '/categories'
    ...
    context 'logged out' do 
      it 'does not let a user view the categories index' do
        get '/categories'
        expect(last_response.location).to include("/login")
      end
    end
